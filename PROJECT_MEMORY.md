...
- `vercel.json`: framework Vite, `npm install --global pnpm@8.9.2 && pnpm install --frozen-lockfile`, `pnpm build`, output `dist/`, rewrite route không có phần mở rộng về `index.html`. Cố định pnpm vì Vercel mặc định chọn 8.15.9, không khớp `engines.pnpm` 8.9.2.
...
- Build React ban đầu thành công. Khi kiểm lại sau cấu hình, có thay đổi đồng thời ngoài lượt deploy: package đổi tên thành `capybara-go-guide`, thêm `src/views/Guide/data/`; build lỗi thiếu `../types` trong `content.ts` và `skills.ts`. Chờ phần chuyển đổi hoàn tất rồi kiểm lại; không coi kết quả build template là bằng chứng cho source mới.
- Đăng nhập Vercel đã được người dùng làm mới. Project `neikops-projects/capybara-go-guide` đã kết nối GitHub `neikop/capybara-go-guide`; framework Vite, Node 24.x. Chuẩn bị push `main` để xác minh deployment tự động.
